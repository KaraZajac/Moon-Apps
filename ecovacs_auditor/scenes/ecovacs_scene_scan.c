...
void ecovacs_scene_scan_on_enter(void* context) {
    ...
    view_dispatcher_switch_to_view(app->view_dispatcher, EcovacsViewLoading);

    gap_set_scan_callback(ecovacs_scan_callback, app);
...
bool ecovacs_scene_scan_on_event(void* context, SceneManagerEvent event) {
    EcovacsAuditorApp* app = context;
    ...
    if(event.event == EcovacsCustomEventTick) {
        if(app->scanning && gap_get_state() != GapStateScanning) {
            ...
            if(app->scan_device_count > 0) {
                scene_manager_next_scene(app->scene_manager, EcovacsSceneScanResults);
            } else {
                scene_manager_previous_scene(app->scene_manager);
            }
        }
...
void ecovacs_scene_scan_on_exit(void* context) {
    EcovacsAuditorApp* app = context;
    furi_timer_stop(app->timer);
    if(app->scanning) {
        gap_stop_scanning();
