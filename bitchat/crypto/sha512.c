...
void sha512_Update(SHA512_CTX* ctx, const uint8_t* data, size_t len) {
    size_t idx = (size_t)(ctx->count[0] >> 3) & 0x7F;
...
void sha512_Final(SHA512_CTX* ctx, uint8_t* hash) {
    ...
    store64_be(&ctx->buf[112], ctx->count[1]);
    store64_be(&ctx->buf[120], ctx->count[0]);
    sha512_transform(ctx, ctx->buf);
    for(int i = 0; i < 8; i++) store64_be(&hash[i * 8], ctx->state[i]);
}
